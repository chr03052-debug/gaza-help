...
const labels = {
  ...
  ja: {
    ...
    shareWarning:
  "注意：このサイトは独立した非公式サイトです。サービスや現地状況は急に変化する可能性があります。リンク確認日は現在のサービス提供を保証するものではありません。判断や移動の前に、必ず最新の公式情報を確認してください。",
...
function isStale(iso, item = null) {
  if (!iso) return false;
  ...
  if (
    item &&
    (
      item.id === "sawa-protection-hotline" ||
      item.id === "ocha-shelter-gaza"
    )
  ) {
    ...
    .map(item => {
      const badge =
        item.status === "sample"
          ...
          : isStale(
              item.content_date || item.verified_at,
              item
            )
